      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <label>Title</label>
            <br>
            <input type="text" name="title" class="form-control" v-bind:placeholder="modalWordPopup.original" v-model="modalWordPopupInput.original">
            <br>
            <label>Translation</label>
            <br>
            <input type="text" name="title" class="form-control" v-bind:placeholder="modalWordPopup.translation" v-model="modalWordPopupInput.translation">
            <br>
            <label>Audio URL</label>
            <br>
            <input type="text" name="title" class="form-control" v-bind:placeholder="modalWordPopup.audio" v-model="modalWordPopupInput.audio">
            <br> 
          </div>
          <div class="modal-footer">
              <button type="button" class="btn btn-primary" @click="updateWordPopup()" v-if="closeFag == false">update</button>
...
export default {
  ...
  methods: {
    ...
    updateWordPopup(){
      ...
      axios.post(CONFIG.BACKEND_URL + '/word_popup/update', formData).then(response => {
        if(response.data.error.status === 100){
          let error = response.data.error.message
          if(error.word !== 'undefined'){
            this.errorMessage = error.word[0]
